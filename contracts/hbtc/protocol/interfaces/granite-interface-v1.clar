...
(define-public (granite-borrow
  ...
  (begin
    ...
    (try! (contract-call? .state check-trading-auth (contract-of borrower-trait) none (some (contract-of asset-trait)) none))
...
(define-public (granite-add-collateral
  ...
  (begin
    (try! (contract-call? .hq-hbtc check-is-trader contract-caller))
    (try! (contract-call? .state check-trading-auth (contract-of borrower-trait) none (some (contract-of collateral-trait)) none))
    (try! (contract-call? .reserve transfer collateral-trait amount this-contract))
...
(define-public (granite-remove-collateral
  ...
  (begin
    ...
    (try! (write-feed price-feed-1))
    (try! (write-feed price-feed-2))
    (try! (contract-call? borrower-trait remove-collateral none collateral-trait amount none))
