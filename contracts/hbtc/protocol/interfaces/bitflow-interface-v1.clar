...
(use-trait stableswap-core .bitflow-traits-v1.stableswap-core-trait)
(use-trait xyk-core .bitflow-traits-v1.xyk-core-trait)
(use-trait swap-helper .bitflow-traits-v1.swap-helper-trait)
...
(define-public (bitflow-stable-swap-x-for-y
  ...
  (begin
    ...
    (try! (contract-call? .state-v1 check-trading-auth (contract-of core-trait) (some (contract-of pool-trait)) (some (contract-of x-token-trait)) (some (contract-of y-token-trait))))

    (try! (contract-call? .reserve-v1 transfer x-token-trait x-amount this-contract))
    (let (
...
(define-public (bitflow-stable-swap-y-for-x
  ...
  (begin
    ...
    (try! (check-is-swap-active))
    (try! (contract-call? .state-v1 check-trading-auth (contract-of core-trait) (some (contract-of pool-trait)) (some (contract-of x-token-trait)) (some (contract-of y-token-trait))))
    (try! (contract-call? .reserve-v1 transfer y-token-trait y-amount this-contract))
    (let (
...
(define-private (calculate-min-output-stableswap
  ...
  (min-output-arg uint))
  (let (
    (input-token-data (contract-call? .state-v1 get-asset (contract-of input-token-trait)))
    (max-slippage (get max-slippage input-token-data))
    (output-token-base (get token-base (contract-call? .state-v1 get-asset (contract-of output-token-trait))))
...
(define-private (get-price-data (is-stablecoin bool) (price-feed-id (buff 32)))
  (if is-stablecoin
    ...
    (contract-call? .pyth-storage-v4 get-price price-feed-id)
  )
)
...
(define-private (check-is-price-stale (publish-time uint))
  (let (
    (block-delay (contract-call? .state-v1 get-block-delay))
...
(define-private (calculate-min-output-xyk
  ...
  (let (
    (input-token-data (contract-call? .state-v1 get-asset (contract-of input-token-trait)))
    (output-token-data (contract-call? .state-v1 get-asset (contract-of output-token-trait)))
    (max-slippage (get max-slippage input-token-data))
...
(define-private (write-feed (price-feed (optional (buff 8192))))
  (match price-feed bytes 
    (begin
      (try! (contract-call? .pyth-oracle-v4 verify-and-update-price-feeds
        bytes
        {
