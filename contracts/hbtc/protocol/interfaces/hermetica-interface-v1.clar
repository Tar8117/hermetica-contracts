...
(define-public (hermetica-unstake-and-withdraw
  ...
  (let (
    ...
    (try! (contract-call? staking-silo withdraw claim-id))
    (try! (contract-call? .usdh-token transfer usdh-amount current-contract reserve none))
...
(define-public (hermetica-redeem
  ...
  (let (
    ...
    (try! (as-contract? ((with-ft usdh-token "*" amount-usdh) (with-stx (+ amount-usdh max-fee-usdh))) 
