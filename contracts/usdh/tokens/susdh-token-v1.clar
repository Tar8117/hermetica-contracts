...
(define-public (transfer (amount uint) (sender principal) (recipient principal) (memo (optional (buff 34))))
  (begin
    ...
    (if (var-get only-protocol) 
      (asserts! (or (contract-call? .hq get-contract-active sender) (contract-call? .hq get-contract-active recipient)) ERR_ONLY_PROTOCOL)
      true
    )

    (if (var-get blacklist-enabled)
      (try! (contract-call? .blacklist-susdh check-is-not-full-blacklist-two sender recipient))
      true
    )
...
(define-public (start-migration (manager <token-migration-trait>))
	(begin
		(try! (is-not-migrated))
		(try! (contract-call? .hq check-is-owner contract-caller))
